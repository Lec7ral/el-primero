...
pygame.display.set_caption("Adivíname el número!")
TEXT = text("", WHITE, ((WIDTH/2), HEIGHT/2 + 50))
TEXT2 = text("Adivíname el número!", RED, (WIDTH/2,HEIGHT/2))
    ...
    if event.unicode == "\x08":
        TEXT.subtract()
            ...
            if str(TEXT.currentText) == str(RANDOM_NUMBER):
                global puntuacion
                puntuacion+=1
                TEXT2.change(f"{TEXT.currentText} Estás en lo correcto! tienes: %i ptos" %(puntuacion))
                mixer.init()
                mixer.music.load('victoria.mp3')
                mixer.music.play()
                RANDOM_NUMBER = random.randint(0,100)
            elif not TEXT.currentText.isdigit():
                TEXT2.change(f"{TEXT.currentText} No es un número! ",)
            elif int(TEXT.currentText) > 100:
                TEXT2.change(f"{TEXT.currentText} Es solo de 0 a 100! ",)
            else:
                TEXT2.change(f"{TEXT.currentText} No es ese! ")
    else:
